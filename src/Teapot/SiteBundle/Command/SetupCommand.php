...
namespace Teapot\SiteBundle\Command;
...
class SetupCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ...
            ->setName('teapot:forum:install')
            ->setDescription('Install the necessary teapot components')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $images = $this->getContainer()->get('teapot.image')->setup();

        $output->writeln('Setting up default groups.');
        $groups = $this->getContainer()->get('teapot.user.group')->setup();
        ...
        $users = $this->getContainer()->get('teapot.user')->setup($username, $email, $password, $groups, $images);

        if ($input->getOption('default-topic-board')) {
            $output->writeln('Setting up default board and topic.');

            $boards = $this->getContainer()->get('teapot.forum.board')->setup($users[0]);

            $topic = $this->getContainer()->get('teapot.forum.topic')->setup($users[0], $boards[0]);
        }
    }
    ...
    {
        $dialog = $this->getDialogHelper();
        $dialog->writeSection($output, 'Installing Teapot Forum');
