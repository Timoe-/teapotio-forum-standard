...
namespace Teapot\ForumBundle\Listener;
...
class RequestListener
{
    ...
    public function onKernelRequest(FilterControllerEvent $event)
    {
        ...
        $this->container->get('teapot.forum.board')->getBoards();

        if ($event->getRequest()->attributes->get('boardSlug') !== null
            && method_exists($controller, 'setBoard') === true) {

            $board = $this->container->get('teapot.forum.path')->lookupBoard();
        ...
        if ($event->getRequest()->attributes->get('topicSlug') !== null
            ...
            if ($useId === true) {
                ...
                $topic = $this->container->get('teapot.forum.topic')->getById($topicId);
            }
            else {
                $topic = $this->container->get('teapot.forum.topic')->getBySlug($topicSlug);
            }
        ...
        if ($event->getRequest()->attributes->get('messageId') !== null
            ...
            $message = $this->container->get('teapot.forum.message')->find($messageId, null);
